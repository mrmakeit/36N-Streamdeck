...
class OBSWS extends EventEmitter {
  ...
  connect(){
    ...
    this.client.connect(this.connectArgs)
      .then(()=>{
        ...
        obs.getSpecialSources(function(data){
          obs.mic = data['mic-1'] 
          obs.pc = data['mic-2'] 
	  obs.client.send('GetMute',{source:obs.pc})
            .then(function(state){
              obs.emit('PCMuteToggled',state['muted']);
	    });
	  obs.client.send('GetMute',{source:obs.mic})
            .then(function(state){
              obs.emit('MicMuteToggled',state['muted']);
	    });
        })
	obs.getCurrentScene(function(name){
          obs.currentScene = name
          obs.emit('SceneSwitch',name);
        })
  ...
  getSpecialSources(done){
    this.client.send('GetSpecialSources',{})
      .then(function(data){done(data)})
      .catch((error) => {
        console.log(error)
      })
  }
  getCurrentScene(done){
    this.client.send('GetCurrentScene',{})
      .then(function(data){console.log(data);done(data.name)})
      .catch((error) => {
        console.log(error)
      })
  }    
  togglePC(){
    let obs = this;
    this.client.send('ToggleMute',{source:this.pc})
      .then(function(){
        return obs.client.send('GetMute',{source:this.pc})
      })
      .then(function(state){
	console.log(state)
      });
  ...
  toggleMic(){
    let obs = this;
    this.client.send('ToggleMute',{source:obs.mic})
      .then(function(){
        return obs.client.send('GetMute',{source:obs.mic})
      })
      .then(function(state){
	console.log(state)
        obs.emit('MicMuteToggled',state['muted']);
